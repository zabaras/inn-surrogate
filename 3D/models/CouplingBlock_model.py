...
class CouplingBlock(nn.Module):
    ...
    def forward(self, x, c, sample_the_data=False):
        ...
        if sample_the_data == False:
            ...
            y2 = (torch.exp(0.636 * torch.atan(self.s_network))) * x2 + self.t_network
            output = torch.cat((x1, y2), 1)
            jacobian2 = torch.sum((0.636 * torch.atan(self.s_network)), tuple(range(1, self.input_len+1)))
            ...
            self.s_network = self.s_net(x1_c)
            self.t_network = self.t_net(x1_c)
            temp = (x2 - self.t_network) / (torch.exp(0.636 * torch.atan(self.s_network)))
            output = torch.cat((x1, temp), 1)
            jacobian1 = torch.sum((0.636 * torch.atan(self.s_network )), dim=tuple(range(1, self.input_len+1)))
